...
class BD: #класс для методов работы с бд
    ...
    def car_bd(self, model, id, frame): #Добавление новой машины в БД
        try:
            ...
            succes_label.place(relx=0.4, rely=0.4, width=500)
        ...
        except Exceptions.BadidError:
            Errorlab = Label(frame, text='Ошибка ввода! id - 6 цифр', bg='blue', fg='red', font=30)
            Errorlab.place(relx=0.4, rely=0.4, width=500)
        finally:
            if (connection3):
...
class InterFace(BD):
    ...
    def view_busy_cars_page(self):
        view_busy_frame = Frame(root, bg='blue')
        view_busy_frame.place(relwidth=1, relheight=1)

        exit_btn = Button(view_busy_frame, text='X', bg='red', fg='white', command=exit, font=200)
        exit_btn.place(relx=0.98, rely=0, height=30, width=40)

        vfcp_title = Label(view_busy_frame, text='Просмотр занятых автомобилей', bg='red', font=30)
        vfcp_title.place(relx=0.32, rely=0.1, width=350)
        list_of_free_cars = self.view_BD(2)
        list_of_offers = self.view_BD(3)
        free_cars = []
        for i in list_of_free_cars:
            for j in list_of_offers:
                if int(i[-1]) == 1 or (int(i[-1]) == 2 and j[0] != self.name and int(i[1]) == int(j[1])):
                    free_cars.append(i)
        free_cars = set(free_cars)
        output_list = []
        for i in free_cars:
        ...
        print(output_list)
        cars_var = StringVar(value=output_list)
        listbox = Listbox(view_busy_frame, listvariable=cars_var)
        ...
        #     new_label.pack()
        back_btn = Button(view_busy_frame, text='Назад', bg='white',
                          command=lambda: (view_busy_frame.destroy(), self.delete_by_time()))
        back_btn.place(relx=0.45, rely=0.7, width=150)
    ...
    def main_page(self, name): #Основная страница пользователя
        ...
        mnbtn22 = Button(main_Frame, text='Просмотреть весь занятый транспорт', bg='white',
                        command=lambda: (self.view_busy_cars_page(), self.delete_by_time()))
        mnbtn22.place(relx=0.41, rely=0.3, width=270)
        ...
        mnbtn3.place(relx=0.41, rely=0.35, width=270)
        ...
        mnbtn4.place(relx=0.41, rely=0.4, width=270)
        ...
        mnbtn5.place(relx=0.41, rely=0.45, width=270)
        ...
        mnbtn6.place(relx=0.41, rely=0.5, width=270)

        mnbtn7 = Button(main_Frame, text='Выйти из системы', bg='white', command=exit)
        mnbtn7.place(relx=0.41, rely=0.55, width=270)
